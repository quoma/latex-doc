...
We are going to use stocks as the containers for each of our population groups. Each stock element in AnyLogic provides a way of keeping track of its value. The value can change depending on the flows that control the flow of population from one group to another (we can see flow elements on Figure \ref{basic22}). We create 3 stocks that are going to be called Susceptible, Resistant and Uncolonized, as you can see on Figure \ref{basic34}.
...
In the next section we are going to add some more flows that will represent the recovery of patient from either susceptible or resistant bacteria strains.